...
extern void initMotor0Int(void);
...
int main(void){  
  ...
  uartTxPoll(UART0, "=============================\n\r");
	...
	initializeADC1(3); 							// Master, configure for optical
	initializeTimer0(800); 					// count of 800 = 10uS.
	initializeTimer1(800);
	initMotor0Int(); 
	...
	while(1) {
		...
		// Check x and y values sent from Master device.
		checkUART();
	
		yVal = (int) YANALOG - 48;
		if(yVal >= 5 && !turn90Left && !turn90Right && !turn180){ 
			checkDutyCycle(yVal, &MotorDutyCycle);
		}
		
		if(opticalCheck){ checkOpticalSensor(); }
		if(finalDistance0 != -1) { checkUltrasonicSensor0(); }
		if(finalDistance1 != -1) { checkUltrasonicSensor1(); }
